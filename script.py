...
for erase_loops in [False, True]:
    for walk_variant in ['R', 'NB', 'LRV', 'NC', 'HS']:
        ...
        print(f"{walk_variant}: avg_mean_hop_count={int(mean(pair_mean_hop_counts)):.0f} avg_median_hop_count={int(mean(pair_median_hop_counts)):.0f} avg_max_hop_count={int(mean(pair_max_hop_counts)):.0f}")
    print()
    print()
    print()