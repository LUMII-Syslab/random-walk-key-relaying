...
def thin_ticks(ticks: list[float], min_sep_s: float = MIN_TICK_SEP_S) -> list[float]:
    if not ticks:
        return ticks
    out = [ticks[0]]
    for x in ticks[1:]:
        if x - out[-1] >= min_sep_s - 1e-9:
            out.append(x)
    return out


def apply_time_ticks_at_completion(
    ax: plt.Axes,
    ...
    ticks = sorted({float(x) for x in completion_times_s if 0.0 <= float(x) <= total_s + 1e-9})
    ticks = thin_ticks(ticks)
    if not ticks:
        ticks = [0.0]
...
def build_threshold_series(
    ...
    targets: list[str],
    thresholds: list[int],
) -> tuple[dict[int, list[float]], dict[int, list[int]], dict[int, float]]:
    max_nodes = len(targets)
    cumulative: dict[str, int] = {tgt: 0 for tgt in targets}
    reached: dict[int, set[str]] = {thr: set() for thr in thresholds}
    completed: dict[int, bool] = {thr: False for thr in thresholds}
    ...
    for i, e in enumerate(events):
        ...
        for thr in thresholds:
            ...
            if len(reached[thr]) >= max_nodes:
                completed[thr] = True
                completion_time[thr] = t[i]

    return xs, ys, completion_time
...
def plot_threshold_reach_counts(
    ...
    xs, ys, completion_time = build_threshold_series(src_node, events, t, targets, thresholds)
    ...
    for i, thr in enumerate(thresholds):
        line_style = line_style_for(i)
        (line,) = ax.plot(xs[thr], ys[thr], **line_style)
        color = line.get_color()
        if thr in completion_time and xs[thr]:
            ax.plot(
                xs[thr][-1],
                ys[thr][-1],
                marker=marker_for(i),
    ...
    apply_time_ticks_at_completion(
        ax,
        [completion_time[thr] for thr in thresholds if thr in completion_time],
        halt_time_s,
    )
    ax.set_ylim(0, max_nodes + 4)
...
def main() -> None:
    ...
    parser.add_argument(
        "--log",
        type=Path,
        default=Path("data/scouted-mar.log"),
        help="Path to scouted log file.",
    )
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("figs/treshold-times.pdf"),
