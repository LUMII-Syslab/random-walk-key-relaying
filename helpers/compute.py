...
_built_scouted = False


def _ensure_scouted_built() -> None:
    global _built_scouted
    if _built_scouted:
        return
    repo_root = _repo_root_from_file()
    subprocess.run(["make", "build/scouted"], cwd=str(repo_root / "cpp"), check=True)
    _built_scouted = True


def _default_maxprobhalttime_cache_path() -> Path:
    repo_root = _repo_root_from_file()
...
def _stable_key(params: MaxProbHaltTimeParams) -> str:
    payload = json.dumps(params.__dict__, sort_keys=True, separators=(",", ":"))
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def _read_json_cache(cache_path: Path) -> dict:
...
def _write_json_cache(cache_path: Path, data: dict) -> None:
...
def _run_scouted_halt_time(params: MaxProbHaltTimeParams) -> float:
    _ensure_scouted_built()
    repo_root = _repo_root_from_file()
    ...
    raise RuntimeError("Could not parse halt time from scouted output")
...
def query_maxprob_halt_time(
    *,
    ...
    cache_file = Path(cache_path) if cache_path is not None else _default_maxprobhalttime_cache_path()
    key = _stable_key(params)

    if not force_recompute:
        cache = _read_json_cache(cache_file)
    ...
    value = _run_scouted_halt_time(params)

    cache = _read_json_cache(cache_file)
    entries = cache.setdefault("entries", {})
    entries[key] = {"ok": True, "value": float(value), "params": params.__dict__}
    _write_json_cache(cache_file, cache)
    return float(value)
