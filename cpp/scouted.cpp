...
namespace cartel
{
        ...
        const vector<char> *hit_nodes = nullptr)
    {
        ...
        for (int v = 0; v < n; v++)
        {
            if (v == src || v == tgt)
                continue;
            if (covered_chunks_by_node[v].none())
                continue;
            candidates.push_back(v);
        }
        if (candidates.empty())
            return res;
        if (static_cast<int>(candidates.size()) < cartel_sz)
            cartel_sz = static_cast<int>(candidates.size());
        if (cartel_sz <= 0)
            return res;
        ...
        res.has_max_cartel_miss = has_miss;
        if (best_a != -1)
            res.nodes = {best_a, best_b, best_c};
        return res;
    }
} // namespace cartel
    ...
    bool path_adds(const vector<int> &path) const
    {
        for (int v : path)
        {
            if (v < 0 || v >= static_cast<int>(has_node.size()))
                return true;
            if (!has_node[static_cast<size_t>(v)])
                return true;
        }
    ...
    int chunk_idx)
{
    for (int x : path)
    {
        if (x == src || x == tgt)
            continue;
        if (x < 0 || x >= static_cast<int>(covered_chunks_by_node.size()))
            continue;
...
static shared_ptr<RwToken> make_walk_token(const Options &opts, int src_idx)
{
    ...
    if (opts.rw_variant == "R")
        return make_shared<RToken>(src_idx, tgt_idx, seed);
    if (opts.rw_variant == "NB")
        return make_shared<NbToken>(src_idx, tgt_idx, seed);
    if (opts.rw_variant == "LRV")
        return make_shared<LrvToken>(src_idx, tgt_idx, seed);
    if (opts.rw_variant == "HS")
        return make_shared<HsToken>(src_idx, tgt_idx, seed);
...
struct CompareEvent
{
    bool operator()(const Event &a, const Event &b) const
    {
        ...
        // smaller time has higher priority: min-heap by time
    }
};
...
void run_simulation(const Options &opts, const Graph &graph)
{
    ...
    // by SK
    ///// DROPPING SCOUTS AFTER SOME TIME /////
    const string MALICIOUS_NODE = "GEN";
    int MALICIOUS_NODE_INDEX = graph.node_index(MALICIOUS_NODE);
    double DROP_START_TIME = 1000; // seconds
    int n_dropped = 0;

    priority_queue<Event, vector<Event>, CompareEvent> pq; // by SK, added CompareEvent to make it a min-heap by time
    ...
    auto remaining_watermark_nodes = [&]() -> vector<string>
    {
        ...
        for (const string &src_name : opts.src_nodes)
        {
            ...
            for (int i = 0; i < graph.node_count(); i++)
            {
                if (i == src)
                    continue;
                if (established_count(src, i) >= opts.required_cnt)
                    continue;
                remaining.push_back(src_name + "->" + graph.node_name(i) + "#" + to_string(established_count(src, i)));
            }
        }
        return remaining;
    };
    ...
    {
        if (!opts.report_yield)
            return;
        cout << fixed << setprecision(6);
        for (const auto &[key, ys] : yield_stats)
        {
            if (ys.input_chunks == 0)
                continue;
            const double rho = static_cast<double>(ys.output_keys) / static_cast<double>(ys.input_chunks);
    ...
    auto ensure_block_state = [&](BlockState &blk)
    {
        ...
        if (
            blk.covered_chunks_by_node.empty() || static_cast<int>(blk.covered_chunks_by_node.size()) != n || (n > 0 && static_cast<int>(blk.covered_chunks_by_node[0].size()) != opts.block_chunks))
        {
    ...
    while (pq.size() > 0)
    {
        ...
        {
            const bool prob_ok = (e.receiver != e.origin && consume(
            ...
            bool accept = false;
            // by SK: after some time start dropping scouts
            if (e.time > DROP_START_TIME && e.receiver == MALICIOUS_NODE_INDEX && e.origin != MALICIOUS_NODE_INDEX)
            {
                n_dropped++;
                // drop the scout
                if (((int)(e.time) % 600 == 0) && ((int)(e.time * 100) % 100 == 0))
                { // every 10 minutes, print how many scouts have been dropped
                    cerr << "Dropped " << n_dropped << " scouts from " << graph.node_name(e.origin) << " to " << graph.node_name(e.receiver) << " at time " << e.time << " seconds" << endl;
                }
                // cerr << "Dropping scout from " << graph.node_name(e.origin) << " to " << graph.node_name(e.receiver) << " at time " << e.time << " seconds" << endl;
                double r = (double)(rng() - rng.min()) / (double)(rng.max() - rng.min());
                /*                if (r < 0.5) {
                                    // drop the scout
                                    continue;
        ...
        if (e.type == EventType::ChunkReceived)
        {
            ...
            if (blk.received == opts.block_chunks)
            {
                ...
                for (auto &bs : blk.covered_chunks_by_node)
                    bs.reset();
                blk.ler.clear();
                ...
                if (opts.verbose)
                {
                    ...
                    for (int v : cr.nodes)
                        cartel_names.push_back(graph.node_name(v));
                    ...
                    cout << "   current buffer size: " << established_count(e.origin, e.target) << endl;
                }
                ...
                if (opts.required_cnt != -1)
                {
                    ...
                    /*else{
                        cerr << "Not halting, remaining=" << remaining_watermark_nodes().size() << endl;
                    }*/
                }
            }
        }
...
static Options parse_args(int argc, char **argv)
{
    ...
    cli.reg_context("--halt-at-keys", [&opts]()
                    { return to_string(opts.required_cnt); });
